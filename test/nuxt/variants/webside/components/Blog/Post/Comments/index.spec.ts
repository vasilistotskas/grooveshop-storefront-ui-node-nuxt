...
import BlogPostComments from '~/components/variants/webside/Blog/Post/Comments/index.vue'
...
describe('BlogPostComments load more', () => {
  ...
  it('loads the next page through the Nuxt proxy route, never the absolute Django URL', async () => {
    const wrapper = await mountSuspended(BlogPostComments, {
      ...
      global: {
        stubs: {
          LazyWebsideBlogPostCommentsList: { template: '<div />' },
          WebsideBlogPostCommentsList: { template: '<div />' },
        },
      },
    })
    await flushPromises()
