...
import StepShipping from '~/components/variants/webside/Checkout/StepShipping.vue'
...
describe('BoxNow Checkout Integration', () => {
  describe('StepShipping within a form context', () => {
    ...
    it('transitions from home_delivery to box_now_locker shows the locker picker section', async () => {
      ...
      // Initially no picker
      expect(wrapper.findComponent({ name: 'WebsideCheckoutSelectedBoxNowLocker' }).exists()).toBe(false)

      // Switch to box_now_locker
      formState.shippingMethod = 'box_now_locker'
      await wrapper.vm.$nextTick()

      expect(wrapper.findComponent({ name: 'WebsideCheckoutSelectedBoxNowLocker' }).exists()).toBe(true)
    })
