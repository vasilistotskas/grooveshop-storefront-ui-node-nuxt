...
<template>
	<div ref="tabs" class="tabs">
		<ClientOnly>
			<div
				class="tabs-header relative overflow-hidden flex space-x-6 text-sm font-bold text-primary-300 bg-primary-700/45 rounded-t-lg px-5 py-3"
			>
				<div
				...
				<span
					...
					class="absolute flex h-full top-0 left-0 p-1 py-1.5 overflow-hidden transition-all duration-300 text-primary-700 dark:text-primary-100"
					:style="{ zIndex: 1 }"
				>
					<span class="flex-1 bg-zinc-500/40 rounded-lg" />
				</span>
			</div>
			<template #fallback>
				<ClientOnlyFallback />
		...
		<div
			class="tabs-body relative text-slate-800 dark:text-white bg-zinc-200 dark:bg-zinc-800 shadow rounded-b-lg"
		>
			<slot />
		</div>
	</div>
