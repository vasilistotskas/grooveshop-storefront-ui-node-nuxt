...
//
// Multi-tenant note: Viva's ``POST /checkout/v2/orders`` only accepts a
// single per-order ``urlFail`` — there is no per-order success URL field.
// Each tenant's Viva merchant portal source success URL must therefore be
// configured (manually, per tenant, as an ops step) to point at that
// tenant's own primary domain, e.g.
// ``https://<tenant-primary-domain>/{locale}/checkout/viva-return``, so
// the redirect below lands back on the originating tenant.
