...
- `scripts/` — `fetch-schema.mjs` (download the OpenAPI schema from Django), `sync-schema-yml.mjs` (regenerate the derived `.yml`), `check-lockstep-deps.mjs`, `audit-visual.mjs`, `capture-ssr.mjs` + `diff-ssr.mjs` (capture a tenant's SSR HTML before/after a change and diff it with build hashes, nonces, generated ids and Cloudflare rewrites normalised — the webside freeze proof; two same-build captures diff to nothing; check the captures are real pages first, a capture of "Store not found" diffs to nothing too), `warm-cache.mjs` (the post-deploy cache warm-up; shipped in the image and run by the infrastructure's PostSync hook: every store's sitemap × device class, page and `_payload.json`)
...
- **App fetchers**: the app calls its own `/api` ONLY through `$api` (for `$fetch`), `useApi` / `useLazyApi` (for `useFetch` / `useLazyFetch`) and `useRequestApi` (for `useRequestFetch`) — `app/utils/api.ts`, `app/plugins/api.ts`, `app/composables/useApi.ts`. They state the page's locale in `X-Language` (`pageLocaleHeader`), which is the language Django answers in and what the Nitro caches key on; an ESLint `no-restricted-syntax` rule fails a raw `$fetch` / `useFetch` / `useLazyFetch` / `useRequestFetch` in `app/**`. Tests mock `$api` / `useApi` with `mockNuxtImport`. On the server `useApi`/`useLazyApi` fetch a key ONCE per render however many readers it has (`app/utils/serverRenderCachedData.ts`: Nuxt's default refetches for every reader that registers after the first fetch settled); a raw `useAsyncData` must spread `serverRenderCachedData()` and set `dedupe: 'defer'` itself. Readers of one key must pass identical options, so a key read from several places gets ONE composable (`useStoreSettings`, `useAllCategories`).
...
- There is deliberately **no env var naming a store.** Which tenant is the platform's own storefront (the one that renders the brand assets bundled under `public/`) is the `TenantConfig.isPlatformStorefront` row flag from `/tenant/resolve`, read through `useTenantStore().isPlatform` / `useIsPlatformTenant()` (app) and `isPlatformTenantConfig(tenant)` (server). The author and site-verification meta are per-store fields on the same payload (`seoAuthor`, `googleSiteVerification`, `pinterestDomainVerify`).
