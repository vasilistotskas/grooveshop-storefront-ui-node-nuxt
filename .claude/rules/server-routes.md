...
- **What reaches the log line**: only fields set on the request logger BEFORE it emits — `server/middleware/0.tenant.ts` (`tenantSchema`, `tenantName`), `server/middleware/2.evlog-auth.ts` (user id). evlog prints the stdout line (what Vector ships to VictoriaLogs) at emit; `evlog:enrich` hooks run after that and feed drains only, and there is no drain. So `server/plugins/evlog-enrichers.ts` (user-agent, geo, request size, trace context) never reaches the logs today (verified 2026-09-25: 0 of 1,004 events carried a field from it).
