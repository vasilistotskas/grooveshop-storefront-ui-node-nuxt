...
One store is the platform's own storefront: it renders the brand assets bundled under `public/` (`img/logo*.png`, `platform-favicon/**`) and the platform-only surfaces (`/llms.txt`). It is designated by the `TenantConfig.isPlatformStorefront` row flag on the resolve payload (`Tenant.is_platform_storefront` in Django, DB-constrained to one row) — never by a hostname comparison and never by an env value: the old `NUXT_PUBLIC_BASE_URL` match put tenant #1's hostname into every tenant's serialized runtime config. Read it through `useTenantStore().isPlatform` / `useIsPlatformTenant()` (app) or `isPlatformTenantConfig(tenant)` (server); an absent tenant counts as platform (probes, prerender). The author and site-verification meta (`seoAuthor`, `googleSiteVerification`, `pinterestDomainVerify`) are per-store fields emitted only where set. Never put a store's hostname, title, logo or tokens in `runtimeConfig.public` — every public key is serialized into every tenant's HTML.
