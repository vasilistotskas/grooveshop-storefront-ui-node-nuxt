...
- **`vi.stubGlobal('$fetch', mock)` does not work at all since Nuxt 4.5** — `$fetch` is a real auto-import in user code (`export { $fetch } from '#build/fetch.mjs'`), so the composable's import binding bypasses globals entirely. App code calls `$api` / `useApi` / `useLazyApi` / `useRequestApi` rather than Nuxt's fetchers, so mock those like any other auto-import: `const { mockFetch } = vi.hoisted(() => ({ mockFetch: vi.fn() }))` + `mockNuxtImport('$api', () => mockFetch)` at module level. `useApi` still transports through Nuxt's `$fetch`, so a test whose `useApi` answers must come from the same mock also mocks `$fetch` — with a `create` (`Object.assign(mockFetch, { create: () => mockFetch })`), because `app/plugins/api.ts` calls `$fetch.create()` while the app boots. Caveat: this mock is active during Nuxt bootstrap (`/api/_auth/session`, `/api/_allauth/app/v1/config`, `/api/cart`) — if the suite needs `$i18n` or store setup, give the mock a default implementation `vi.fn(() => Promise.resolve({}))` so the plugin chain doesn't crash.
