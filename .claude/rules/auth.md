...
- **Reading a session creates one**: `getUserSession` on a request with no session mints a new one and sets `nuxt-session` (h3 `getSession`). Code that only needs to KNOW whether there is a session (logging, gates on every request) must check `requestHasSession(event)` (`server/utils/auth.ts`) first. Until 2026-09-26 the log-tagging middleware did not, so every anonymous page set a cookie, and a response that sets a cookie is never cached at the edge. Sessions are created where one is needed: login, the cart (`server/utils/cartSession.ts`).
