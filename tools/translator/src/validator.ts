...
const isRequired = <T>(v: T): v is NonNullable<T> =>
  v !== undefined && v !== null
...
const isNonNegativeNumber = (v: any): v is number =>
  typeof v === 'number' && v >= 0
  ...
  rules.forEach(({ property, validators, optional }) => {
    ...
    if (propertyPath in obj || propertyPath.includes('.')) {
      validators.forEach(({ validator, errorMsg }) => {
        if (!validator(value)) {
          throw new Error(
            `Config validation error at '${propertyPath}': ${errorMsg}`,
          )
        }
      })
    } else if (!optional) {
    ...
    [
      {
        ...
        errorMsg: `'sourceFileName' is a required field.`,
      },
...
const settingsParamValidationRules: Rule<ISettingsParam<any>>[] = [
    ...
    [
      {
        ...
        errorMsg: `'parentNames' must be an array of strings.`,
      },
    ],
    true,
  ),
  ...
  createRule(
    ...
    [
      {
        validator: isString,
        errorMsg: `'argumentsArrayName' must be a string.`,
      },
    ],
    true,
  ),
  ...
  createRule(
    ...
    [
      {
        validator: isString,
        errorMsg: `'prettyLogTemplate' must be a string.`,
      },
    ],
    true,
  ),
  createRule(
    ...
    [
      {
        validator: isString,
        errorMsg: `'prettyErrorTemplate' must be a string.`,
      },
    ],
    true,
  ),
  createRule(
    ...
    [
      {
        validator: isString,
        errorMsg: `'prettyErrorStackTemplate' must be a string.`,
      },
    ],
    true,
  ),
  ...
  createRule(
    ...
    [
      {
        validator: isBoolean,
        errorMsg: `'stylePrettyLogs' must be a boolean.`,
      },
    ],
    true,
  ),
  ...
  createRule(
    ...
    [
      {
        validator: isObject,
        errorMsg: `'prettyInspectOptions' must be an object.`,
      },
    ],
    true,
  ),
  ...
  createRule(
    ...
    [
      {
        validator: isObject,
        errorMsg: `'attachedTransports' must be an object.`,
      },
    ],
    true,
  ),
