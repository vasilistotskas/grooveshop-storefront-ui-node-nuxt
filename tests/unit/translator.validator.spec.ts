...
describe('translator validator tests', () => {
	...
	it('createRule should create a rule object correctly', () => {
		const rule = createRule('test', [{ validator: isString, errorMsg: 'Error' }])
		expect(rule).toEqual({
			property: 'test',
	...
	it('should throw an error for missing required fields', () => {
		...
		expect(() => validateRules(incompleteConfig, configValidationRules)).toThrow()
	})
	it('should throw an error for invalid nested property values', () => {
		...
		expect(() => validateRules(invalidNestedConfig, configValidationRules)).toThrow()
	})
	it('should validate a correct configuration without errors', () => {
		expect(() => validateRules(sampleConfig, configValidationRules)).not.toThrow()
	})
