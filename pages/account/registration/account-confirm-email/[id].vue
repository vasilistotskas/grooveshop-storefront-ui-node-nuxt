...
function onSubmit(values: RegistrationResendEmailBody) {
  registrationResendEmail({
    ...
    .then(() => {
      toast.add({
        title: t(
          'pages.account.registration.account-confirm-email.resend.success.title',
        ),
        color: 'green',
      })
    })
      ...
      if (error) {
        toast.add({
          title:
            error.value?.message
            ?? t(
              'pages.account.registration.account-confirm-email.resend.error.title',
            ),
          color: 'red',
        })
      }
...
const formSchema: DynamicFormSchema = {
  fields: [
    {
      label: t(
        'pages.account.registration.account-confirm-email.resend.form.email.label',
      ),
      name: 'email',
      as: 'input',
