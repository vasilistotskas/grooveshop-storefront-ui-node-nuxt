...
import {
  ...
  type AcceptClientHintsHeadersKey,
  AcceptClientHintsRequestHeaders,
  HTTPClientHints,
  HttpRequestHeaders,
  type SSRClientHints,
...
export default defineNuxtPlugin({
  name: 'client-hints:server:plugin',
  order: -25,
  parallel: true,
  setup() {
    ...
    const clientHintsRequest = collectClientHints(
      userAgent,
      ssrClientHintsConfiguration,
      requestHeaders,
    )
    // 3. write client hints response headers
    writeClientHintsResponseHeaders(
      clientHintsRequest,
      ssrClientHintsConfiguration,
    )
...
type BrowserFeatures = Record<
  AcceptClientHintsHeadersKey,
  BrowserFeatureAvailable
>
...
function browserFeatureAvailable(
  userAgent: ReturnType<typeof parseUserAgent>,
  feature: AcceptClientHintsHeadersKey,
) {
...
function lookupClientHints(
  ...
  if (userAgent == null || userAgent.type !== 'browser') return features

  if (ssrClientHintsConfiguration.prefersColorScheme)
    features.prefersColorSchemeAvailable = browserFeatureAvailable(
      userAgent,
      'prefersColorScheme',
    )

  if (ssrClientHintsConfiguration.prefersReducedMotion)
    features.prefersReducedMotionAvailable = browserFeatureAvailable(
      userAgent,
      'prefersReducedMotion',
    )

  if (ssrClientHintsConfiguration.viewportSize) {
    features.viewportHeightAvailable = browserFeatureAvailable(
      userAgent,
      'viewportHeight',
    )
    features.viewportWidthAvailable = browserFeatureAvailable(
      userAgent,
      'viewportWidth',
    )
  }
...
function collectClientHints(
  ...
  const hints: SSRClientHints = lookupClientHints(
    userAgent,
    ssrClientHintsConfiguration,
  )
  ...
  if (ssrClientHintsConfiguration.prefersColorScheme) {
    if (ssrClientHintsConfiguration.prefersColorSchemeOptions) {
      ...
      if (cookieValue) {
        ...
        if (
          ssrClientHintsConfiguration.prefersColorSchemeOptions.themeNames.includes(
            value,
          )
        ) {
...
function writeClientHintHeaders(
  key: string,
